...
CStage_10Desper::CStage_10Desper(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CLevel_Stage{ pDevice, pContext }
{
}

HRESULT CStage_10Desper::Initialize()
{
	...
	_wstring strLoadPath = L"../Bin/Resources/_SUPERHOT/_BinaryLevels/Stage_10Desper.datmap";
...
void CStage_10Desper::Update(_float fTimeDelta)
{
	...
	if (pFrontEnemy == nullptr)
	{
		if (m_iPhase < 2)
		{
			m_iPhase++;
			if (FAILED(Ready_Layer_Monster(TEXT("Layer_Monster"))))
				MSG_BOX(L"[CStage_10Desper:Update] Monster Spawn Failed. Cur Phase : " + m_iPhase);
...
HRESULT CStage_10Desper::Ready_Layer_Camera(const _wstring& strLayerTag)
{
	_uint iDestLevel = m_pGameInstance->Get_DestLevel();
	...
	//	ENUM_CLASS(LEVEL::CH10_DESPER), TEXT("Prototype_GameObject_Camera_Free"), &CameraDesc)))
	//	return E_FAIL;
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(iDestLevel, strLayerTag,
		ENUM_CLASS(LEVEL::CH10_DESPER), TEXT("Prototype_GameObject_Camera_Player"), &CameraDesc)))
		return E_FAIL;
...
HRESULT CStage_10Desper::Ready_Layer_BackGround(const _wstring& strLayerTag)
{
	//if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::CH10_DESPER), strLayerTag,
	...
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::CH10_DESPER), strLayerTag,
		ENUM_CLASS(LEVEL::CH10_DESPER), TEXT("Prototype_GameObject_Skybox"))))
		return E_FAIL; // !!

	return S_OK;
}

HRESULT CStage_10Desper::Ready_Layer_UI(const _wstring& strLayerTag)
{
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::CH10_DESPER), strLayerTag + L"_Crosshair",
		ENUM_CLASS(LEVEL::STATIC), TEXT("Prototype_GameObject_UI_Crosshair"))))
		return E_FAIL;

	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::CH10_DESPER), strLayerTag + L"_ScreenText",
		ENUM_CLASS(LEVEL::STATIC), TEXT("Prototype_GameObject_UI_ScreenText"))))
		return E_FAIL;

	m_pUI_ScreenText = dynamic_cast<CUI_ScreenText*>(m_pGameInstance->Get_LastGameObject(ENUM_CLASS(LEVEL::CH10_DESPER), strLayerTag + L"_ScreenText"));
	CLevel_Stage::m_pUI_ScreenText = m_pUI_ScreenText;

	return S_OK;
}

HRESULT CStage_10Desper::Ready_Layer_Player(const _wstring& strLayerTag)
{
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::CH10_DESPER), strLayerTag,
		ENUM_CLASS(LEVEL::STATIC), TEXT("Prototype_GameObject_Player"))))
		return E_FAIL;
	CGameObject* pPlayer = m_pGameInstance->Get_LastGameObject(ENUM_CLASS(LEVEL::CH10_DESPER), strLayerTag);

	_float4 pPos = { 153.287f, 5.f, 17.8562f, 1.f };
	dynamic_cast<CTransform*>(pPlayer->Get_Component(L"Com_Transform"))->Set_State(STATE::POSITION, XMLoadFloat4(&pPos));
	dynamic_cast<CTransform*>(pPlayer->Get_Component(L"Com_Transform"))->Rotation(XMVectorSet(0.f, 1.f, 0.f, 1.f), TO_RAD(176));

	return S_OK;
}

HRESULT CStage_10Desper::Ready_Layer_Monster(const _wstring& strLayerTag)
{
	...
	case 1:	// Phase 1
	{
		// 2-1 정면 멀리에 벽 뒤에서 걸어옴
		vStartPos = { 191.526f, 5.f, 89.6058f, 1.f };
		EnemyDesc.iFirstCellIndex = 109;
		fStartRotDeg = -159.496f;

		EnemyDesc.vecPremovePoses = { { 173.527f, 5.f, 80.4563f } };
		...
		// 2-2 화장실 나오자마자 정면
		vStartPos = { 184.514f, 5.f, 62.5643f, 1.f };
		EnemyDesc.iFirstCellIndex = 78;
		fStartRotDeg = -135.907f;
		...
		// 2-3 카운터에서 대기중인 적
		vStartPos = { 120.723f, 5.f, 67.1466f, 1.f };
		EnemyDesc.iFirstCellIndex = 42;
		fStartRotDeg = 98.2913f;
	...
	case 2:
	{
		// 3-1 카운터 뒤 왼쪽 스폰구에서 생성 후 걸어나옴
		vStartPos = { 102.873f, 5.f, 27.9114f, 1.f };
		EnemyDesc.iFirstCellIndex = 99;
		fStartRotDeg = 8.69626f;

		EnemyDesc.vecPremovePoses = { {107.639f, 5.f, 46.7837f} };
		EnemyDesc.iDefaultWeaponObjType = ENUM_CLASS(GAMEOBJ_TYPE::WEAPON_RANGED_PISTOL);
		...
		// 3-2 카운터 뒤 스폰구
		vStartPos = { 89.9222f, 5.f, 69.6028f, 1.f };
		EnemyDesc.iFirstCellIndex = 86;
		fStartRotDeg = 112.301f;

		EnemyDesc.vecPremovePoses = { 
			{ 99.9493f, 5.f, 69.0318f },
			{ 103.707f, 5.0053f, 83.746f },
			{ 128.377f, 5.01465f, 85.0776f }		
		};
		EnemyDesc.iDefaultWeaponObjType = ENUM_CLASS(GAMEOBJ_TYPE::END);
		...
		// 3-3 카운터 반대방향 스폰구 (문 막힌곳)
		vStartPos = { 184.448f, 5.f, 50.9251f, 1.f };
		EnemyDesc.iFirstCellIndex = 78;
		fStartRotDeg = -77.9751f;
...
HRESULT CStage_10Desper::Ready_Layer_Effect(const _wstring& strLayerTag)
{
	//if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::CH10_DESPER), strLayerTag,
	//	ENUM_CLASS(LEVEL::CH10_DESPER), TEXT("Prototype_GameObject_Particle_TriEffect"))))
	//	return E_FAIL;
...
HRESULT CStage_10Desper::Ready_Pickup_Objects(const _wstring& strLayerTag)
{
...
{
	CStage_10Desper* pInstance = new CStage_10Desper(pDevice, pContext);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX(TEXT("Failed to Created : CStage_10Desper"));
